...
uint8   flag_camera_suan = 0;  //采集图像中开始计算的标志位
...
void ov7725_eagle_vsync(void)
{
    ...
    if(ov7725_eagle_img_flag == IMG_START)                   //需要开始采集图像
    {
        ...
        PORTA_ISFR = 1 <<  PT27;            //清空PCLK标志位
  ImgChange = ImgNow;                                   //指针翻转
       ImgNow = ImgAgo;
       ImgAgo = ImgChange;
       
          flag_camera_suan = 1;
           PORTA_ISFR = 1 <<  PT27;          //清空PCLK标志位
        DMA_DADDR(CAMERA_DMA_CH) = (uint32)ImgNow;  //恢复地址
...
void ov7725_eagle_get_img()
{
 ...
 while(1)//等待flag_camera_suan变为1
  {
    if(flag_camera_suan==1)
    {
